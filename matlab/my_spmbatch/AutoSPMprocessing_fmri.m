...
params.analysisname = 'ME-EFT-denn_perTE';
...
%% fMRI data parameters
    params.preprocfmridir = 'preproc_func_denn_me-perTE'; %directory with the preprocessed fMRI data
    params.fmri_prefix = 'sdwaure'; %fMRI file name of form [fmri_prefix 'sub-ii_task-..._' fmri_endfix '.nii']
    
    params.dummytime = 0;
    
    %In case of multiple runs in the same session exist
    params.mruns = false; %true if run number is in filename
    params.runs = [1]; %the index of the runs (in filenames run-(index))
    ...
    params.meepi = true; % true or false for ME-fMRI (if true, _echo-... should be in the file name)
    params.echoes = [1]; %list of echoes for ME-fMRI used as sessions in SPM
