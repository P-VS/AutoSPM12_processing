...
for ti=1:nvols:tdim
    ...
    if do_pepolar
        if ~exist('funcdat','var')
            ...
            funcdat = spm_read_vols(tVfunc(ti:ti+nvols-1)); 
            dim = size(funcdat);
            if numel(dim)<4, funcdat = reshape(funcdat,[dim(1),dim(2),dim(3),1]); end
        end
    
        [funcdat,ppparams,delfiles,keepfiles] = my_spmbatch_pepolar(funcdat,numdummy,enum,ti,ppparams,delfiles,keepfiles);
    
        if ti==1, prefix = ['u' prefix]; end
    end
