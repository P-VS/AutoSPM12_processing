...
/* [FA, EV1, EV2, EV3, EW, outnr] = acid_c_dti_to_ev_ew (VOL) */
...
void VoxelPos ( double *FA, double *iVOL, const mwSize *dims, double *EV1, double *EV2, double *EV3, double *EW, double *outlV)
{
    ...
    for (i.xa = 0; i.xa < dims[0]; i.xa++)
    {
        ...
        if((dM[0]+dM[1]+dM[2]) >0) 
        {
            ...
            EV1[pos_E]  = eigenvectors[eig1][1];
            EV2[pos_E]  = eigenvectors[eig2][1];
            EV3[pos_E]  = eigenvectors[eig3][1];
            pos_E      = pos + 1*dims[0];
            EW[pos_E]  = eigenvalues[eig2];
            EV1[pos_E]  = eigenvectors[eig1][2];
            EV2[pos_E]  = eigenvectors[eig2][2];
            EV3[pos_E]  = eigenvectors[eig3][2];
            pos_E      = pos + 2*dims[0];
            EW[pos_E]  = eigenvalues[eig3];
            EV1[pos_E]  = eigenvectors[eig1][3];
            EV2[pos_E]  = eigenvectors[eig2][3];
            EV3[pos_E]  = eigenvectors[eig3][3];
