/* acid_c_dti_to_ev_ew.c */

/* [FA, EVx, EVy, EVz, EW, outnr] = acid_c_dti_to_ev_ew (VOL) */
...
void VoxelPos ( double *FA, double *iVOL, const mwSize *dims, double *EVx, double *EVy, double *EVz, double *EW, double *outlV)
{
    ...
    for (i.xa = 0; i.xa < dims[0]; i.xa++)
    {
        ...
        if((dM[0]+dM[1]+dM[2]) >0) 
        {
            ...
            eig1        = max1(eigenvalues); /* index of largest eigenvector*/ 
            eig2        = max2(eigenvalues); /* index of 2nd largest eigenvector*/ 
            eig3        = max3(eig1,eig2);   /* index of 3rd largest eigenvector*/ 

			/* EW is the same vector as eigenvalues but the eigenvalues are in decr#easing order */
			/* EVx contains the x coordinates of all 3 eigenvectors, where eigenvectors are ordered by decreasing eigenvalue */
			/* EVy contains the y coordinates of all 3 eigenvectors, where eigenvectors are ordered by decreasing eigenvalue */
			/* EVz contains the z coordinates of all 3 eigenvectors, where eigenvectors are ordered by decreasing eigenvalue */
            pos_E      = pos;
            EW[pos_E]  = eigenvalues[eig1];
            EVx[pos_E] = eigenvectors[1][eig1];
            EVy[pos_E] = eigenvectors[2][eig1];
            EVz[pos_E] = eigenvectors[3][eig1];
            pos_E      = pos + 1*dims[0];
            EW[pos_E]  = eigenvalues[eig2];
            EVx[pos_E] = eigenvectors[1][eig2];
            EVy[pos_E] = eigenvectors[2][eig2];
            EVz[pos_E] = eigenvectors[3][eig2];
            pos_E      = pos + 2*dims[0];
            EW[pos_E]  = eigenvalues[eig3];
            EVx[pos_E] = eigenvectors[1][eig3];
            EVy[pos_E] = eigenvectors[2][eig3];
            EVz[pos_E] = eigenvectors[3][eig3];
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *FA, *VOL, *EVx, *EVy, *EVz, *EW, *outlV; 
   ...
   EVy    = mxGetPr (plhs[2]);
    ...
    VoxelPos (FA,VOL, dims, EVx, EVy, EVz, EW,outlV);    
}
