function VO = my_spmbatch_spm_reslice_rt(P,flags)
...
for i = 2%1:numel(P)
    ...
    if read_vol
        if ~isfinite(flags.interp)
            v = abs(kspace3d(spm_bsplinc(P(i), [0 0 0 ; 0 0 0]'), ...
                                                       P(1).mat\P(i).mat));
            for x3 = 1:P(1).dim(3)
                if flags.mean
                    Integral(:,:,x3) = ...
                        Integral(:,:,x3) + ...
                        nan2zero(v(:,:,x3) .* ...
                        getmask(inv(P(1).mat\P(i).mat), x1, x2, x3, ...
                                                P(i).dim(1:3),flags.wrap));
                end
                if flags.mask
